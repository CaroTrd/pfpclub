...
partnersRoutes.get('/partners', function (req, res, next) {
  ...
  JOIN partners_types ON partnership_type_id=pa.partnership_type ORDER BY partnership_type DESC`, (err, results) => {
      console.log(err)
      if (err) {
        res.status(500).send('Erreur lors de la récupération');
      } else {
...
partnersRoutes.get('/partners-level', function (req, res, next) {
  connexion.query(
    ...
    JOIN companies AS co ON co.company_id = pa.company_id`,
    (err, results) => {
      if (err) {
        console.log(err);
